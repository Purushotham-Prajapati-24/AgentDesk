...
export default async function EmbedPage({ params, searchParams }: EmbedPageProps) {
  ...
  const { theme, position, className, parentOrigin } = await searchParams;
  ...
  const cleanParentOrigin = parentOrigin && /^https?:\/\/[a-zA-Z0-9.-]+(:\d+)?$/.test(parentOrigin) ? parentOrigin : undefined;
    ...
    <main className="h-svh w-full overflow-hidden">
      {safeBotId ? (
        <script
          ...
          data-parent-origin={cleanParentOrigin}
          nonce={cleanCspNonce}
          async
        />
      ) : (
